	...
	*/
	var tougao = {
			...
			that._submit.bind('click', function(){
				...
				$.ajax({
				    ...
				    success: function(data){
				        ...
				        if(data === 'content'){
				        	_tip.show().html('内容不能为空，且介于'+_deel.tougaoContentmin+'-'+_deel.tougaoContentmax+'个字符之间！')
				        }
				    }
				});
			})
		...
		check_content: function(target){
			if(!target.val() || target.val().length < _deel.tougaoContentmin){
				this.tip(target,'内容太短，不得少于'+_deel.tougaoContentmin+'字', 2)
			}
			else if(target.val().length > _deel.tougaoContentmax){
				this.tip(target,'内容太长，不得超过'+_deel.tougaoContentmax+'字', 2)
			}
			else{
